...
type ScoringAccolades = Omit<Accolades, "roy_won"> & { roy_won: boolean | number };

type ClassicStatKey = "ppg" | "rpg" | "apg" | "spg" | "bpg";
type ClassicStatLine = Partial<Record<ClassicStatKey, number | null>>;
...
type ClassicPointBlock = TeamEra & {
  points?: number;
...
export default function Home() {
  ...
  const [rosterFeedView, setRosterFeedView] = useState<RosterFeedView>("mixed");
  const [rosterSort, setRosterSort] = useState<RosterSort>("pra");
    ...
    () =>
      (hasActiveDraftSelection ? players : [])
        ...
        .sort((a, b) => {
          ...
          const priorityDelta = rosterSortDelta(a, b, rosterSort, selection);

          if (priorityDelta) {
            return priorityDelta;
          }

          const legacyDelta = playerLegacyScore(b, selection) - playerLegacyScore(a, selection);
  ...
  const simulateSeason = useCallback(() => {
    ...
    const payload: ClassicResultPayload = {
      mode: "classic",
      selectedTeam,
      ...
      lineup: lineupEntries.map(({ position, player, selection }) => ({
        ...
        selection,
        achievements: buildAchievements(player, selection),
        positionBonus: positionBonusForSlot(lineup[position], position),
      })),
      totals: lineupAchievementTotals,
    };

    sessionStorage.setItem(CLASSIC_RESULT_STORAGE_KEY, JSON.stringify(payload));
    setStatus(`${result.tier}: ${result.wins}-${result.losses}`);
    router.push("/classic/results");
  }, [
    activeEra,
    lineup,
            ...
            <>
              <div className="border-b border-white/10 px-4 py-4">
                ...
                <div className="roster-filter-row mt-4 flex flex-wrap items-center gap-2">
                  ...
                  <select
                    aria-label="Roster feed view"
                    className="h-10 w-[116px] rounded-lg border border-white/12 bg-[#242938] px-3 text-sm font-black normal-case tracking-normal text-white outline-none transition focus:border-[#31d6a1] focus:ring-2 focus:ring-[#31d6a1]/20"
                    value={rosterFeedView}
                    onChange={(event) => setRosterFeedView(event.target.value as RosterFeedView)}
                  >
                    {ROSTER_FEED_VIEW_OPTIONS.map((view) => (
                  ...
                  >
                    {ROSTER_SORT_OPTIONS.map((filter) => (
                    ...
                    {filteredPlayers.map((player) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <AchievementStrip
                            achievements={buildRosterFeedAchievements(player, playerSelection, rosterFeedView)}
                          />
                        </button>
                      );
                    })}
        ...
        <section className="game-court-section flex flex-col gap-4 self-start">
          ...
          {isAdmin ? (
            <div className="game-score-panel flex flex-col gap-3 rounded-lg border border-[#ff8a2a]/25 bg-[#202431] p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#ff8a2a]">Classic Score</p>
...
function CourtSlot({
  ...
  const courtAchievements = player ? buildAchievements(player, slot?.selection).slice(0, 5) : [];
